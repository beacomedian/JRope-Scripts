-- @description kusa_Setup kH Compactor from selected to hovered track
-- @version 1.02
-- @author Kusa, modified by Jesse Rope
...
function main()
    ...
    trckspcr_id = reaper.TrackFX_AddByName(dest_tr, 'kHs Compactor (Kilohearts)', false, 1)
    if trckspcr_id == -1 then
        reaper.ShowMessageBox("kHs Compactor is not installed.", "Error", 0)
        return
    end
...
main()
reaper.Undo_EndBlock("Create Folder Structure From Selected Tracks", -1)
