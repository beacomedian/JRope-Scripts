 ...
 * About:
   # Moves the take FX from each selected item up onto the item's track.
     ...
     are deleted. Moved FX are inserted ahead of any FX already on the track
     (keeping their original relative order). Complementary to
 ...
 * Changelog:
  ...
  # v1.2 - Moved item FX are now inserted ahead of existing track FX instead of
  #        appended at the end.
 * To Do:
  #
...
-- (tracked in `state.seen`), the FX is moved up onto `track`; every later
-- duplicate is deleted from the take. Always read from take index 0 because both
-- a move and a delete remove that FX, sliding the next one down to 0.
--
-- Moved FX are inserted ahead of any pre-existing track FX: each kept FX goes to
-- `state.next` (0 for the first), which pushes existing track FX further down,
-- and `state.next` advances so the moved FX keep their original relative order
-- while staying in front of whatever was already on the track.
local function consolidate_item_fx(item, track, state)
  ...
  while reaper.TakeFX_GetCount(take) > 0 do
    ...
    if state.seen[sig] then
      reaper.TakeFX_Delete(take, 0)  -- duplicate settings: discard
      discarded = discarded + 1
    else
      state.seen[sig] = true
      reaper.TakeFX_CopyToTrack(take, 0, track, state.next, true)  -- true = move
      state.next = state.next + 1  -- next moved FX lands right after this one
      moved = moved + 1
    end
  end
...
function main()
  ...
  -- State is scoped per track: each track keeps its own set of seen FX
  -- signatures (so a unique FX lands on every track it appears on, while
  -- duplicates among that track's items collapse to a single instance) plus a
  -- running insert index `next`, starting at 0 so moved FX go ahead of any FX
  -- already on the track.
  local state_by_track = {}

  local total_moved, total_discarded = 0, 0
  for item in EnumSelectedItems() do
    local track = reaper.GetMediaItemTrack(item)
    local state = state_by_track[track]
    if not state then
      state = { seen = {}, next = 0 }
      state_by_track[track] = state
    end
    local moved, discarded = consolidate_item_fx(item, track, state)
